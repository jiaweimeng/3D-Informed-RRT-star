...
/**
*  get the ID of the point when we know the coordinate
**/
int RRT::getID(double X, double Y, double Z)
{
  int i, returnID=0;
  for(i=0;i<this->getTreeSize();i++)
  {
    if (X == rrtTree[i].posX && Y == rrtTree[i].posY && Z == rrtTree[i].posZ)
    {
      returnID = i;
    }
  }
...
*/
void RRT::setPosZ(int nodeID, double input_PosZ)
{
    rrtTree[nodeID].posZ = input_PosZ;
}
